...
const AjustarStockSiropeModal = ({ show, onHide, sirope, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      await siropesService.ajustarStock(
        sirope.id_sirope,
        {
    ...
    <Modal show={show} onHide={handleClose} centered>
      <Modal.Header closeButton>
        <Modal.Title>
          ...
          Ajustar Stock - {sirope.nombre_sirope}
        </Modal.Title>
      </Modal.Header>
      <Form onSubmit={handleSubmit}>
        <Modal.Body>
          <Alert variant="info">
            <strong>Stock actual:</strong> {sirope.stock_actual} {sirope.unidad_medida}
          </Alert>
          ...
          {formData.cantidad && (
            <Alert variant={nuevoStock >= 0 ? 'success' : 'danger'}>
              <strong>Nuevo stock:</strong> {nuevoStock.toFixed(2)} {sirope.unidad_medida}
              {nuevoStock < 0 && (
                <div className="mt-2">
...
export default AjustarStockSiropeModal;